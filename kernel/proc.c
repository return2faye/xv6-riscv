...
found:
  ...
  // Allocate a USYSCALL shared page
...
proc_pagetable(struct proc *p)
{
  ...
  // map USYSCALL page (read-only to user)
  // USYSCALL is a user VA (defined in memlayout.h).
  // We map p->usyscall's PA (on riscv xv6 KVA==PA for RAM) with PTE_U|PTE_R (no write/exec).
  if(mappages(pagetable, USYSCALL, PGSIZE,
              (uint64)p->usyscall, PTE_W | PTE_R) < 0){
